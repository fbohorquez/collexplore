...
export const SortableCard = ({ id, text, index, moveCard }) => {
	...
	const [{ handlerId }, drop] = useDrop({
		...
		drop(item, monitor) {
			...
			if (typeof dragIndex !== "string" || dragIndex.indexOf("_label") !== -1 || typeof hoverIndex !== "string" || hoverIndex.indexOf("_label") !== -1) {
				return;
			}
	...
	return (
		<div ref={ref} class="input-sortable-option" style={{ opacity }} data-handler-id={handlerId}>
			{text}
		</div>
	);
};
